...
const HallOfFame: React.FC<PageProps<DataProps>> = ({ data, path }) => {
  const getData = e => {
    ...
    if (!target.classList.contains("show--hof")) target.classList.add("show--hof")
    ...
    for (var i = 0; i < season.length; i++) {
      if (!season[i].classList.contains(name)) {
        ...
        if (season[i].classList.contains("show--hof"))
          season[i].classList.remove("show--hof")
      }
    }
        ...
        <div className="content-wrapper">
          ...
          {HallOfFameData.content.map(val => {
            ...
            if (val.year + "--" + val.semester == "2021--Winter")
              data = val.year + "--" + val.semester + " season--wrap show--hof"
